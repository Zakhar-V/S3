...
//----------------------------------------------------------------------------//
bool Json::Load(const char* _filename)
{
	auto _src  = arctic::easy::ReadFile(_filename);

	String _err;
	if (!Parse((const char*)_src.data(), &_err))
	{
		LOG("%s%s", _filename, _err.c_str());
		return false;
	}
	return true;
}
//----------------------------------------------------------------------------//
void Json::Save(const char* _filename)
{
	String _str = Print();
	//arctic::easy::WriteFile(_filename, (const uint8*)_str.c_str(), _str.length()); // TODO(Huldra): Implement this
	{
		FILE* _f = fopen(_filename, "wb");
		if (_f)
		{
			fwrite(_str.c_str(), 1, _str.length(), _f);
			fclose(_f);
		}
		else
		{
			LOG("Unable to create file \"%s\"", _filename);
		}
	}
}
//----------------------------------------------------------------------------//
